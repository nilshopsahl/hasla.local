...
class Amasty_Shopby_Block_Adminhtml_Filter_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($model->getBackendType() == 'decimal') {
        	$this->_prepareDecimalForm();
        } else {
    ...
    /**
     * Prepare form for filters having decimal type
     */
    protected function _prepareDecimalForm()
    {
    	$hlp = Mage::helper('amshopby');
        $yesno = array(Mage::helper('catalog')->__('No'), Mage::helper('catalog')->__('Yes'));
        ...
        $fldSet->addField('block_pos', 'select', array(
         ...
         $fldSet->addField('from_to_widget', 'select', array(
          'label'     => $hlp->__('Show From-To Widget'),
          'name'      => 'from_to_widget',
          'values'    => $yesno
        ));
