    ...
    if ($_REQUEST['email']) {
          ...
          try {
                ...
                if ($ownerId !== null && $ownerId != $customerSession->getId()) {
                    echo $this->__('This email address is already assigned to another user.').'&&0';
                    return;
                }

                $status = Mage::getModel('newsletter/subscriber')->subscribe($email);
                if ($status == Mage_Newsletter_Model_Subscriber::STATUS_NOT_ACTIVE) {
                   echo $this->__('Confirmation request has been sent.')."&&1";
                    return;
                }
                else {
                   echo $this->__('Thank you for your subscription.')."&&1";
                   return;
                }
            }
