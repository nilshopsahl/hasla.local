...
class Trollweb_BBSNetAxept_Model_Api_Bbs extends Varien_Object
{
   ...
   public function query($TransactionId)
   {
     ...
     if ($response) {
       ...
       $this->Result()->setExpiryDate((string)$response->CardInformation->ExpiryDate);
       $this->Result()->setPanHash((string)$response->Recurring->PanHash);
...
//       'paymentMethodList'  => $this->getPaymentMethodList(), // TODO: implement this
    ...
    $config = Mage::helper("bbsnetaxept/config");

    if ($config->easypaymentIsEnabled()) {
      $request["recurringType"] = "S";

      // Add Pan Hash if the customer selected a stored easypayment card
      if ($this->customerDidSelectEasypaymentCard()) {
        $entry = Mage::helper("bbsnetaxept/panhash")->load();
        if ($entry) {
          $request["panHash"] = $entry->getPanhash();
        }
      }
    }
  ...
  private function customerDidSelectEasypaymentCard() {
    return Mage::getSingleton('checkout/session')->getNetsEasypaymentCard();
  }
